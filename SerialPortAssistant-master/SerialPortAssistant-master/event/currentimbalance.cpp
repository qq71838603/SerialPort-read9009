    ...
    ui(new Ui::currentimbalance)
{
    ...
    QStringList strList;
    strList<<"上1次电流不平衡记录"<<"上2次电流不平衡记录"<<"上3次电流不平衡记录"<<"上4次电流不平衡记录"<<"上5次电流不平衡记录"<<"上6次电流不平衡记录"<<"上7次电流不平衡记录"<<"上8次电流不平衡记录"<<"上9次电流不平衡记录"<<"上10次电流不平衡记录";
...
void currentimbalance::on_query_button_clicked()
{
    //初始的4个数据标识
    int va0 = 0x01,va1 = 0x00 ,va2 = 0x0A,va3 = 0x03;
    //处理后的4个数据标识
...
//查询码组装函数
QString currentimbalance::get_Check_code(QString DI3,QString DI2,QString DI1,QString DI0)
{
    //将发送过来的数据标识转换为16进制整型
