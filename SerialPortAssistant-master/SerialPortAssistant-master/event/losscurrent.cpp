    ...
    ui(new Ui::losscurrent)
{
    ...
    QStringList strList;
    strList<<"上1次A相失流记录"<<"上2次A相失流记录"<<"上3次A相失流记录"<<"上4次A相失流记录"<<"上5次A相失流记录"<<"上6次A相失流记录"<<"上7次A相失流记录"<<"上8次A相失流记录"<<"上9次A相失流记录"<<"上10次A相失流记录"
             <<"上1次B相失流记录"<<"上2次B相失流记录"<<"上3次B相失流记录"<<"上4次B相失流记录"<<"上5次B相失流记录"<<"上6次B相失流记录"<<"上7次B相失流记录"<<"上8次B相失流记录"<<"上9次B相失流记录"<<"上10次B相失流记录"
               <<"上1次C相失流记录"<<"上2次C相失流记录"<<"上3次C相失流记录"<<"上4次C相失流记录"<<"上5次C相失流记录"<<"上6次C相失流记录"<<"上7次C相失流记录"<<"上8次C相失流记录"<<"上9次A相失流记录"<<"上10次C相失流记录";
...
void losscurrent::on_query_button_clicked()
{
    //初始的4个数据标识
    int va0 = 0x01,va1 = 0x01 ,va2 = 0x0B,va3 = 0x03;
    //处理后的4个数据标识
...
//查询码组装函数
QString losscurrent::get_Check_code(QString DI3,QString DI2,QString DI1,QString DI0)
{
    //将发送过来的数据标识转换为16进制整型
