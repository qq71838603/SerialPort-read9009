    ...
    ui(new Ui::Overvoltage)
{
    ...
    QStringList strList;
    strList<<"上1次A相过压记录"<<"上2次A相过压记录"<<"上3次A相过压记录"<<"上4次A相过压记录"<<"上5次A相过压记录"<<"上6次A相过压记录"<<"上7次A相过压记录"<<"上8次A相过压记录"<<"上9次A相过压记录"<<"上10次A相过压记录"
             <<"上1次B相过压记录"<<"上2次B相过压记录"<<"上3次B相过压记录"<<"上4次B相过压记录"<<"上5次B相过压记录"<<"上6次B相过压记录"<<"上7次B相过压记录"<<"上8次B相过压记录"<<"上9次B相过压记录"<<"上10次B相过压记录"
               <<"上1次C相过压记录"<<"上2次C相过压记录"<<"上3次C相过压记录"<<"上4次C相过压记录"<<"上5次C相过压记录"<<"上6次C相过压记录"<<"上7次C相过压记录"<<"上8次C相过压记录"<<"上9次A相过压记录"<<"上10次C相过压记录";
...
void Overvoltage::on_query_button_clicked()
{
    //初始的4个数据标识
    int va0 = 0x01,va1 = 0x01 ,va2 = 0x03,va3 = 0x03;
    //处理后的4个数据标识
...
//查询码组装函数
QString Overvoltage::get_Check_code(QString DI3,QString DI2,QString DI1,QString DI0)
{
    //将发送过来的数据标识转换为16进制整型
